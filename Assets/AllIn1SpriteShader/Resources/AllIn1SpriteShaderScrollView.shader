Shader "AllIn1SpriteShader/AllIn1SpriteShaderScrollView"
{
	Properties
	{
        ...
        _ShineRotate("Rotate Angle (in radians)", Range(0, 6.2831)) = 0 //132
        _ShineWidth("Shine Width", Range(0.05,1)) = 0.1 // 133
        _ShineGlow("Shine Glow", Range(0,100)) = 1 // 134
		_ShineMask("Shine Mask", 2D) = "white" {} // 135

		_GlitchSize("Glitch Size", Range(0.25, 5)) = 1 //136
		_HologramStripeColor("Hologram Stripes Color", Color) = (1,1,1,1) //137
		...
		SubShader
		{
			...
			Pass
			{
				...
				half4 frag(v2f i) : SV_Target
				{
					...
					#if TEXTURESCROLL_ON
					#if ATLAS_ON
					i.uv = half2(_MinXUV + ((_MaxXUV - _MinXUV) * (abs((_Time.y * _TextureScrollXSpeed) + uvRect.x) % 1)),
					_MinYUV + ((_MaxYUV - _MinYUV) * (abs((_Time.y * _TextureScrollYSpeed) + uvRect.y) % 1)));
					#else
