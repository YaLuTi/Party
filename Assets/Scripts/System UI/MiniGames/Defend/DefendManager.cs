...
public class DefendManager : MiniPlayerControl
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < StageManager.players.Count; i++)
        {
            ...
            g.GetComponent<DefendPlayer>().Set(StageManager.players[i], i);
            g.GetComponent<DefendPlayer>().ClothNum = StageManager.players[i].GetComponent<PlayerIdentity>().ClothNum;
            g.GetComponent<DefendPlayer>().Playernum = i;
    ...
    void PlayerJoin(GameObject player, int num)
    {
        ...
        GameObject g = Instantiate(KungFuPlayer, p, Quaternion.Euler(r));
        g.GetComponent<MiniPlayerControl>().Set(player, num);
        PlayerLifes++;
        Debug.Log(PlayerLifes);
        g.GetComponent<MiniPlayerControl>().ClothNum[0] = num;
        g.GetComponentInChildren<PlayerCreating>().CreatMini();
        players.Add(g);
    }
